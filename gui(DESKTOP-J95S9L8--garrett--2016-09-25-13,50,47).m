...
handles.message.String = '检测人脸或者用鼠标标出目标'; 
guidata(hObject, handles);
...
handles.Timers.timerDetect = timer('Name', 'timer_detect', 'ExecutionMode', 'fixedSpacing', 'Period', 0.01 );
handles.Timers.timerDetect.StartFcn = @(~,~)timerB_start(hObject);
handles.Timers.timerDetect.TimerFcn = @(~,~)timerB_running(hObject);
handles.Timers.timerDetect.ErrorFcn = @(~,~)delete(handles.Timers.timerDetect);
...
handles.Timers.timerTrace.StopFcn = @(~,~)start(handles.Timers.timerDetect);
...
if  isvalid(handles.camObject.cam)
    ...
    handles.tracker.camespx = w / 8;
    handles.tracker.camespy = h / 8;
    ...
    %-------------------------
    %% Classifier parameters
    handles.tracker.clfparams.width = trparams.initstate(3);
    handles.tracker.clfparams.height= trparams.initstate(4);
    % feature parameters
    % number of rectangle from 2 to 4.
    ...
    %% Compute feature template
    [handles.tracker.ftr.px,handles.tracker.ftr.py,handles.tracker.ftr.pw,handles.tracker.ftr.ph,handles.tracker.ftr.pwt] = HaarFtr(clfparams,ftrparams,M);
    %% Compute sample templates
    handles.tracker.posx.sampleImage = sampleImgDet(img,initstate,trparams.init_postrainrad,1);
    handles.tracker.negx.sampleImage = sampleImg(img,initstate,1.5*trparams.srchwinsz,4+trparams.init_postrainrad,trparams.init_negnumtrain);
    %% Feature extraction
    iH = integral(img);%Compute integral image
    handles.tracker.posx.feature = getFtrVal(iH,handles.tracker.posx.sampleImage,ftr);
    handles.tracker.negx.feature = getFtrVal(iH,negx.sampleImage,ftr);
    [handles.tracker.posx.mu,handles.tracker.posx.sig,negx.mu,negx.sig] = classiferUpdate(handles.tracker.posx,negx,handles.tracker.posx.mu,handles.tracker.posx.sig,handles.tracker.negx.mu,handles.tracker.negx.sig,lRate);% update distribution parameters
...
function x = timerB_running(hObject)
    handles = guidata(hObject)
        ...
        elseif ~strcmp(handles.tracker.dirt, handles.tracker.dir)
            ...
            img = snapshot(handles.camObject.cam);
            pause(0.05);
            PelcoD_Rotate(handles.PLZPort.PLZs, '00',dir);
        else
            PelcoD_Stop(handles.PLZPort.PLZs);
            img = snapshot(handles.camObject.cam);
            PelcoD_Rotate(handles.PLZPort.PLZs, '00',dir);
        ...
        r = ratioClassifier(handles.tracker.posx,handles.trakcer.negx,detectx.feature);% compute the classifier for all samples
        clf = sum(r);% linearly combine the ratio classifiers in r to the final classifier
        [c,index] = max(clf);
        x = detectx.sampleImage.sx(index);
        y = detectx.sampleImage.sy(index);
        ...
        detectx.feature = getFtrVal(iH,detectx.sampleImage,ftr);
        r = ratioClassifier(handles.tracker.posx,negx,detectx.feature);% compute the classifier for all samples
        clf = sum(r);% linearly combine the ratio classifiers in r to the final classifier
        [c,index] = max(clf);
        x = detectx.sampleImage.sx(index);
        y = detectx.sampleImage.sy(index);
...
%         hold off;
        ...
        %% Update all the features
        handles.tracker.posx.feature = getFtrVal(iH,handles.tracker.posx.sampleImage,ftr);
        handles.tracker.negx.feature = getFtrVal(iH,handles.tracker.negx.sampleImage,ftr); 
        [handles.tracker.posx.mu,handles.tracker.posx.sig,handles.tracker.negx.mu,handles.tracker.negx.sig] = classiferUpdate(handles.tracker.posx,handles.tracker.negx,handles.tracker.posx.mu,handles.tracker.posx.sig,handles.tracker.negx.mu,handles.tracker.negx.sig,lRate);% update distribution parameters  
        %% 更新参数
        centerx = x + w/2;
        centery = y + h/2;
        camt = camt + 1;
