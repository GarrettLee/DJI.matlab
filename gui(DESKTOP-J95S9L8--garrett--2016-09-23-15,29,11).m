...
% Last Modified by GUIDE v2.5 23-Sep-2016 15:20:39
...
handles.camObject.index = str2double(handles.cam_index.String);
handles.PLZPort.port = handles.port.String{handles.port.Value} ;
handles.PLZPort.bandrate = str2double(handles.bandrate.String);
handles.PLZPort.index = handles.port.Value;
%% webcam、串行口和参数初始化
handles.PLZPort.PLZs = getOpenedPort(handles.PLZPort.port,handles.PLZPort.bandrate);
...
handles.Timers.timerDisplay = Timer('TimerFcn', @(x)timerA_running(handles), 'fixedRate' );
...
%% 更新相机
if handles.camObject.index ~= handles.cam_index.Value | handles.camObject.cam.Resolution ~= handles.resolution.String{handles.resolution.Value}
...
function timerA_running(handles)
     img = snapshot(handles.camObject.cam);
     set(handles.hImage, 'CData', uint8(img), 'CDataMapping', 'scaled', 'XData', [ 1, 640], 'YData', [1, 480]); %XData、YData设置显示的大小
     drawnow;
